...
void R_AliasSetUpTransform(i32 trivial_accept);
        ...
        for (i = 0; i < 12; i++) {
            ...
            if (pv0->flags ^ pv1->flags) {
                frac = (ALIAS_Z_CLIP_PLANE - pa0->fv[2]) /
                       (pa1->fv[2] - pa0->fv[2]);
                viewaux[numv].fv[0] =
                    pa0->fv[0] + (pa1->fv[0] - pa0->fv[0]) * frac;
                viewaux[numv].fv[1] =
                    pa0->fv[1] + (pa1->fv[1] - pa0->fv[1]) * frac;
...
void R_AliasTransformFinalVert(finalvert_t* fv, auxvert_t* av,
    ...
    if (lightcos < 0) {
        temp += (i32) (r_shadelight * lightcos);
...
void R_AliasTransformAndProjectFinalVerts(finalvert_t* fv, stvert_t* pstverts) {
    ...
    for (i = 0; i < r_anumverts; i++, fv++, pverts++, pstverts++) {
        ...
        zi = 1.0 /
             (DotProduct(pverts->v, aliastransform[2]) + aliastransform[2][3]);
...
void R_AliasSetupSkin(void) {
    ...
    if (pskindesc->type == ALIAS_SKIN_GROUP) {
        paliasskingroup =
            (maliasskingroup_t*) ((byte*) paliashdr + pskindesc->skin);
        pskinintervals =
        ...
        skintargettime =
            skintime - ((i32) (skintime / fullskininterval)) * fullskininterval;
...
void R_AliasSetupFrame(void) {
    ...
    if (paliashdr->frames[frame].type == ALIAS_SINGLE) {
        r_apverts =
            (trivertx_t*) ((byte*) paliashdr + paliashdr->frames[frame].frame);
        return;
    }

    paliasgroup =
        (maliasgroup_t*) ((byte*) paliashdr + paliashdr->frames[frame].frame);
...
void R_AliasDrawModel(alight_t* plighting) {
    ...
    // cache align
    pfinalverts = (finalvert_t*) (((intptr_t) &finalverts[0] + CACHE_SIZE - 1) &
                                  ~(CACHE_SIZE - 1));
